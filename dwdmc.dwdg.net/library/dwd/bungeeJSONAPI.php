...
class BungeeJSON {
     ...
     */
    public function __construct($host, $port, $key, $timeout = 1) {
        $this->host = $host;
        $this->port = $port;
        $this->key = $key;
    ...
    public function makeURL($method, array $args) {
        return sprintf(self::URL_FORMAT, $this->host, $this->port, $method, rawurlencode(json_encode($this->constructCall($method, $args))));
    }
